...
from report.db_admin import add_expense, update_expense, get_expense, get_expenses, get_expense_categories
...
def test_get_expenses_returns_dph_columns(tmp_path):
    conn = _setup(tmp_path)
    cat_id = get_expense_categories(conn)[0]["id"]
    add_expense(conn, {
        "property_slug": "p1", "year": 2026, "month": 4,
    ...
    rows = get_expenses(conn, "p1", 2026, 4)
    assert len(rows) == 1
    assert rows[0]["amount_net_czk"] == 100.0
    assert rows[0]["amount_dph_czk"] == 21.0
    assert rows[0]["vat_rate"] == 0.21
